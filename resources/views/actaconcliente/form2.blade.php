...
{{
    ...
    `Rev_Fiscal`, `Nom_Revisor`, `Id_Revisor`, `TP_Revisor`, `Sup_Nom_Revisor`, `IdSup_Revisor`, `TPSup_Revisor`, `Contac_equi_Rev`,`fecharenov`,`idtipo_sociedad`,`sucursal`,`respoiva`,`exportador`,`auditoria`,`contratesp`,`revfis`,`conrevfis`,`indif`, `rettituven` ,
    `facturas`, `ds`, `equel`, `notasdyc`, `nomina`, `dcruce`, `manefectivo`, `manejica`, `maneceret`, 
    `proceavaluos`, `pencliente`, `penContabilidad`, `penlegales`, `pendian`, `pendistritos`,
     `pendsupersociedades`, `pendabogados`, `pendpqrs`, `pendauditoria`, `pendotros`, `ultimedit`,`fechaedit` FROM `datos_actacliente` WHERE `Nit`='.$cliente.'';
   //  echo $query2; 
    $conn = mysqli_connect($database_host,$database_user,$database_password,$database_name);
    ...
    while($row=mysqli_fetch_array($result2)){
       ...
       // echo "Actividade camara:".$Act_Eco;
        ...
        $indif=$row["indif"];
        $rettituven=$row["rettituven"];
        $facturas=$row["facturas"];
        $ds=$row["ds"];
        $equel=$row["equel"];
        $notasdyc=$row["notasdyc"];
        $nomina=$row["nomina"];
        $dcruce=$row["dcruce"];
        $manefectivo=$row["manefectivo"];
        $manejica=$row["manejica"];
        $maneceret=$row["maneceret"];
        $proceavaluos=$row["proceavaluos"];
        $pencliente=$row["pencliente"];
        $penContabilidad=$row["penContabilidad"];
        $penlegales=$row["penlegales"];
        $pendian=$row["pendian"];
        $pendistritos=$row["pendistritos"];
        $pendsupersociedades=$row["pendsupersociedades"];
        $pendabogados=$row["pendabogados"];
        $pendpqrs=$row["pendpqrs"];
        $pendauditoria=$row["pendauditoria"];
        $pendotros=$row["pendotros"];

        $ultimedit=$row["ultimedit"];
        $fechaedit=$row["fechaedit"];
...
$query2="select `perfil_idperfil`  from usuario_web where usuario ='.$idusuario.'";
    //echo $query; 
    $conn = mysqli_connect($database_host,$database_user,$database_password,$database_name);
    $result2=mysqli_query($conn,$query2);
    while($row=mysqli_fetch_array($result2)){
...
<div class="form-group row">
    ...
    <div class="col-lg-8">
    <input type="text" name="fechaedit" id="fechaedit" class="form-control" value="{{old('fechaedit', $fechaedit ?? '')}}" required readonly/>
     </div>
    <label for="id" class="col-lg-3 col-form-label requerido">NIT</label>
    <div class="col-lg-8">
    <input type="text" name="id" id="id" class="form-control" value="{{old('id', $id ?? '')}}" required readonly/>
    </div>
    <label for="fecha_cons" class="col-lg-3 col-form-label requerido">Fecha de Constitución</label>
    <div class="col-lg-8">
    <input type="date" name="fecha_cons" id="fecha_cons" class="form-control" value="{{old('fecha_cons', $fecha_constitucion ?? '')}}" required readonly/>
     </div>
    <label for="nombre" class="col-lg-3 col-form-label requerido">Nombre</label>
    <div class="col-lg-8">
    <input type="text" name="nombre" id="nombre" class="form-control" value="{{old('nombre', $nombre ?? '')}}" required readonly/>
    </div>
    <label for="direccion" class="col-lg-3 col-form-label requerido">Dirección</label>
    <div class="col-lg-8">
    <input type="text" name="direccion" id="direccion" class="form-control" value="{{old('direccion', $direccion ?? '')}}" required readonly/>
    </div>
    ...
    <div class="col-lg-8">
    <input type="text" name="gruniif" id="gruniif" class="form-control" value="{{old('gruniif', $Grupo_NIIF ?? '')}}" />
    </div>
    ...
    <div class="col-lg-8">
    <input type="text" name="capitalext" id="capitalext" class="form-control" value="{{old('capitalext', $Cap_Ext ?? '')}}" />
    </div>
    <label for="respoiva" class="col-lg-3 col-form-label ">Resposable de IVA</label>
    <div class="col-lg-8">
        <select id="respoiva" name="respoiva" class="form-control" >
            <option></option>   
            @foreach( $sino as $key => $value )
            @if ($respoiva==$value)
            <option value="{{ $value }}" selected=selected>{{ $key }}</option>
    ...
    <div class="col-lg-8">
    <input type="text" name="persona_contacto" id="persona_contacto" class="form-control" value="{{old('persona_contacto', $persona_contacto ?? '')}}" />
    </div>
    <label for="cel_contacto" class="col-lg-3 col-form-label requerido">Celular contacto</label>
    <div class="col-lg-8">
     <input type="text" name="cel_contacto" id="cel_contacto" class="form-control" value="{{old('cel_contacto', $tel_movil ?? '')}}" />
    </div>
    <label for="tel_contacto" class="col-lg-3 col-form-label requerido">Telefono contacto</label>
    <div class="col-lg-8">
     <input type="text" name="tel_contacto" id="tel_contacto" class="form-control" value="{{old('tel_contacto', $tel_fijo ?? '')}}" />
    </div>
    <label for="exportador" class="col-lg-3 col-form-label ">Es exportador</label>
    <div class="col-lg-8">
        <select id="exportador" name="exportador" class="form-control" >
            <option></option>   
            @foreach( $sino as $key => $value )
            @if ($exportador==$value)
            <option value="{{ $value }}" selected=selected>{{ $key }}</option>
    ...
    <div class="col-lg-8">
        <select id="auditoria" name="auditoria" class="form-control" >
            <option></option>   
            @foreach( $sino as $key => $value )
            @if ($auditoria==$value)
            <option value="{{ $value }}" selected=selected>{{ $key }}</option>
    ...
    <div class="col-lg-8">
        <input type="text" name="actecon" id="actecon" class="form-control" value="{{old('tel_contacto', $Act_Eco ?? '')}}" />
    </div>
    <label for="contratesp" class="col-lg-3 col-form-label ">CONTRATOS ESPECIALES:</label>
    <div class="col-lg-8">
        <select id="contratesp" name="contratesp" class="form-control" >
            <option></option>   
            @foreach( $sino as $key => $value )
            @if ($contratesp==$value)
            <option value="{{ $value }}" selected=selected>{{ $key }}</option>
    ...
    <div class="col-lg-8">
    <input type="date" name="fecharenov" id="fecharenov" class="form-control" value="{{old('fecharenov', $fecharenov ?? '')}}" required/>
     </div>
     ...
     <div class="col-lg-8">
     <input type="text" name="replegalsup" id="replegalsup" class="form-control" value="{{old('replegalsup', $Sup_Rep_Leg ?? '')}}" />
     </div>
     <label for="idreplegalsup" class="col-lg-3 col-form-label requerido">ID REPRESENTANTE LEGAL SUPLENTE:</label>
     <div class="col-lg-8">
     <input type="text" name="idreplegalsup" id="idreplegalsup" class="form-control" value="{{old('idreplegalsup', $IdSup_Rep_Leg ?? '')}}" />
     </div>
     <label for="propuestaact" class="col-lg-3 col-form-label requerido">PROPUESTA ACTUALIZADA:</label>
     <div class="col-lg-8">
     <input type="text" name="propuestaact" id="propuestaact" class="form-control" value="{{old('propuestaact', $Propuesta ?? '')}}" />
     </div>
    ...
    <div class="col-lg-8">
        <select id="indif" name="indif" class="form-control" >
            <option></option>   
            @foreach( $sino as $key => $value )
            @if ($indif==$value)
            <option value="{{ $value }}" selected=selected>{{ $key }}</option>
    ...
    <div class="col-lg-8">
        <select id="rettituven" name="rettituven" class="form-control" >
            <option></option>   
            @foreach( $sino as $key => $value )
            @if ($rettituven==$value)
            <option value="{{ $value }}" selected=selected>{{ $key }}</option>
    ...
    <div class="form-group">
        ...
        <textarea name="facturas" rows="2" cols="20" class="form-control" placeholder="Facturas electrónicas...">@php echo nl2br($facturas) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas"> DS</label>
        <textarea name="ds" rows="2" cols="20" class="form-control" placeholder=" DS...">@php echo nl2br($ds) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas"> Equel</label>
        <textarea name="equel" rows="2" cols="20" class="form-control" placeholder="Equel...">@php echo nl2br($equel) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas"> Notas débito y crédito</label>
        <textarea name="notasdyc" rows="2" cols="20" class="form-control" placeholder="Notas débito y crédito...">@php echo nl2br($notasdyc) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas"> Documentos de nómina</label>
        <textarea name="nomina" rows="2" cols="20" class="form-control" placeholder="Documentos de nómina...">@php echo nl2br($nomina) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas"> Documentos de cruce</label>
        <textarea name="dcruce" rows="2" cols="20" class="form-control" placeholder="Documentos de cruce...">@php echo nl2br($dcruce) @endphp</textarea>
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="manefectivo" rows="2" cols="20" class="form-control" placeholder="Manejo y control de pagos en efectivo....">@php echo nl2br($manefectivo) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Procedimiento para emisión de certificados bimestrales de Ica.</label>
        <textarea name="manejica" rows="2" cols="20" class="form-control" placeholder="Procedimiento para emisión de certificados bimestrales de Ica....">@php echo nl2br($manejica) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Procedimiento y formatos para solicitud de certificados de retención en la fuente anuales.</label>
        <textarea name="maneceret" rows="2" cols="20" class="form-control" placeholder="Procedimiento y formatos para solicitud de certificados de retención en la fuente anuales....">@php echo nl2br($maneceret) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Procedimientos y momentos en que se solicitan los avalúos técnicos.</label>
        <textarea name="proceavaluos" rows="2" cols="20" class="form-control" placeholder="Procedimientos y momentos en que se solicitan los avalúos técnicos...">@php echo nl2br($proceavaluos) @endphp</textarea>
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="pencliente" rows="2" cols="20" class="form-control" placeholder="Pendientes del cliente...">@php echo nl2br($pencliente) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Contabilidad</label>
        <textarea name="penContabilidad" rows="2" cols="20" class="form-control" placeholder="Pendientes de Contabilidad...">@php echo nl2br($penContabilidad) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Legales</label>
        <textarea name="penlegales" rows="2" cols="20" class="form-control" placeholder="Pendientes de Legales...">@php echo nl2br($penlegales) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas"> Dian</label>
        <textarea name="pendian" rows="2" cols="20" class="form-control" placeholder="Pendientes  Dian...">@php echo nl2br($pendian) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Distrito o municipios</label>
        <textarea name="pendistritos" rows="2" cols="20" class="form-control" placeholder="Pendientes Distrito o municipios...">@php echo nl2br($pendistritos) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Supersociedades</label>
        <textarea name="pendsupersociedades" rows="2" cols="20" class="form-control" placeholder="Pendientes Supersociedades...">@php echo nl2br($pendsupersociedades) @endphp</textarea>
    </div>

    <div class="form-group">
        <label for="Notas">Abogados</label>
        <textarea name="pendabogados" rows="2" cols="20" class="form-control" placeholder="Pendientes Abogados...">@php echo nl2br($pendabogados) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">PQRS</label>
        <textarea name="pendpqrs" rows="2" cols="20" class="form-control" placeholder="Pendientes PQRS...">@php echo nl2br($pendpqrs) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Auditoria</label>
        <textarea name="pendauditoria" rows="2" cols="20" class="form-control" placeholder="Pendientes Auditoria...">@php echo nl2br($pendauditoria) @endphp</textarea>
    </div>
    <div class="form-group">
        <label for="Notas">Otros</label>
        <textarea name="pendotros" rows="2" cols="20" class="form-control" placeholder="Pendientes Otros..."> @php echo nl2br($pendotros) @endphp</textarea>
    </div>
    ...
    <div class="col-lg-8">
    <input type="hidden" name="usuario_id" id="usuario_id" class="form-control" value="{{old('usuario_id', $usuario_id ?? '')}}" required/>
    </div>
