...
{{
    $database =Config::get('database.connections.'.Config::get('database.default'));
        ...
        $queryact="SELECT `Id`, `cliente_idcliente`, `Tipo_clave`, `Clave`, `usuario`, `segclave`, 
        `pregunta`, `respuesta`, `pregunta2`, `respuesta2`, `pregunta3`, `respuesta3`, `correoaso`, `observaciones`
         from `clientes_has_claves` WHERE `Id`='$idrespuesta'";
        //dd($queryact);
        $resultbusq=mysqli_query($conn,$queryact);

        while ($row = mysqli_fetch_array($resultbusq)){
        $id=$row["cliente_idcliente"];
        $usuario=$row["usuario"];
        $tiporet=$row["Tipo_clave"];
        $clave=$row["Clave"];
        $segclave=$row["segclave"];
        ...
        $correoaso=$row["correoaso"];
        $observaciones=$row["observaciones"];
    }

    $queryc="SELECT `nombre` FROM `cliente` WHERE `id`='$id'";
...
<label for="tipo" class="col-lg-3 col-form-label requerido">Plataforma</label>
    ...
    <div class="col-lg-8">
        <input type="text" name="idrespuesta" id="idrespuesta" class="form-control" value= "{{$idrespuesta}}" hidden/>
    </div>
