...
{{
    ...
    $querycert="SELECT * FROM `tipo_solic_cert_bimensuales`";
    $database =Config::get('database.connections.'.Config::get('database.default'));
    $database_name=$database['database'];
    $database_host = $database['host'];
    $database_password =  $database['password'];
...
@endphp
    ...
    <label for="diasencargado" class="col-lg-3 col-form-label requerido">Días habiles para avisar al encargado</label>
    <div class="col-lg-8">
    <input type="text" name="diasencargado" id="diasencargado" class="form-control" value="{{old('diasencargado', $diasencargado ?? '')}}" required/>
    </div>
    <label for="diascliente" class="col-lg-3 col-form-label requerido">Días habiles para avisar al cliente</label>
    <div class="col-lg-8">
    ...
    <label for="certificado" class="col-lg-3 col-form-label requerido">Tipo</label>
    <div class="col-lg-8">
        <select id="certificado" name="certificado" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadoscert))
        <option value="{{ $rowcert["idtipo_solic_cert_bimensuales"] }}">{{ $rowcert["descripcion"] }}</option>    
        @endwhile
        </select>
    </div>
