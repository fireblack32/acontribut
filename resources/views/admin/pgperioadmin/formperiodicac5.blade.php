...
{{
    ...
    $querycert="SELECT * FROM `tipo_renov_reg_inv_ext`";
    $database =Config::get('database.connections.'.Config::get('database.default'));
    $database_name=$database['database'];
    $database_host = $database['host'];
    $database_password =  $database['password'];
...
@endphp
    ...
    <label for="municipio" class="col-lg-3 col-form-label requerido">Tipo </label>
    <div class="col-lg-8">
        <select id="certificado" name="certificado" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadoscert))
        <option value="{{ $rowcert["idtipo_renov_reg_inv_ext"] }}">{{ $rowcert["descripcion"] }}</option>    
        @endwhile
        </select>
    </div>
