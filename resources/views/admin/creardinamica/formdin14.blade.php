@php
{{
    $querydec="SELECT `id_tipo_municipio`, `Municipio`, `Departamento` FROM `tipo_municipio` order by `Municipio`";
...
<label for="tipodeclaracion" class="col-lg-3 col-form-label requerido">Municipio</label>
    <div class="col-lg-8">
        <select id="tipomun" name="tipomun" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadosdec))
        <option value="{{ $rowcert["id_tipo_municipio"] }}">{{ $rowcert["Municipio"]}}-{{$rowcert["Departamento"] }}</option>    
        @endwhile
        </select>
    </div>
