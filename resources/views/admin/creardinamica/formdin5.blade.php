@php
{{
    $querydec="SELECT `idtipo_iva`,`descripcion` FROM `tipo_iva` ORDER BY `descripcion`";
...
<label class="col-lg-8" class="col-lg-3 col-form-label requerido">Número de Digitos</label>
...
<label for="tipodeclaracion" class="col-lg-3 col-form-label requerido">Tipo IVA</label>
    <div class="col-lg-8">
        <select id="tipoiva" name="tipoiva" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadosdec))
        <option value="{{ $rowcert["idtipo_iva"] }}">{{ $rowcert["descripcion"] }}</option>    
        @endwhile
        </select>
    </div>
...
<label for="numero" class="col-lg-3 col-form-label requerido">Números:</label>
    <div class="col-lg-8">
