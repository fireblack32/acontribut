@php
{{
    $querydec="SELECT `idtipo_rete_fuente`,`descripcion` FROM `tipo_rete_fuente` ORDER BY `descripcion`";
...
<label for="tipodeclaracion" class="col-lg-3 col-form-label requerido">Tipo Declaración</label>
    <div class="col-lg-8">
        <select id="tipodeclaracion" name="tipodeclaracion" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadosdec))
        <option value="{{ $rowcert["idtipo_rete_fuente"] }}">{{ $rowcert["descripcion"] }}</option>    
        @endwhile
        </select>
    </div>
