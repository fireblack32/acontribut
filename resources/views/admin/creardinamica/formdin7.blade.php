@php
{{
    $querydec="SELECT `idtipo_impuesto_patrimonio`,`descripcion` FROM `tipo_impuesto_patrimonio` ORDER BY `descripcion`";
...
<label for="tipodeclaracion" class="col-lg-3 col-form-label requerido">Tipo Impuesto al Patrimonio:</label>
    <div class="col-lg-8">
        <select id="tipo" name="tipo" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadosdec))
        <option value="{{ $rowcert["idtipo_impuesto_patrimonio"] }}">{{ $rowcert["descripcion"] }}</option>    
        @endwhile
        </select>
    </div>
