@php
{{
    $querydec="SELECT `idtipo_super_sociedades`, `descripcion` FROM `tipo_super_sociedades` ORDER BY `descripcion`";
...
<label for="tipodeclaracion" class="col-lg-3 col-form-label requerido">Tipo Superintendencia de sociedades: </label>
    <div class="col-lg-8">
        <select id="tipo" name="tipo" class="form-control" required>
        @while ($rowcert = mysqli_fetch_array($resultadosdec))
        <option value="{{ $rowcert["idtipo_super_sociedades"] }}">{{ $rowcert["descripcion"] }}</option>    
        @endwhile
        </select>
    </div>
